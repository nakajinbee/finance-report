...
以降は`facts`本体の取得（書類取得API）を「どの書類を取ればよいか」が既知の状態から
開始できるようにする（`docs/requirements/cycle9_requirements.md` FR-48）。

`facts`（数値データそのもの）とは別テーブルで、`facts`にまだ取り込んでいない書類も
含めて保持する（`facts_ingested_at`で取り込み済みかを判定する）。
...
| csv_flag | String(1) | VARCHAR(1) | | | | CSV有無フラグ。`1`でない書類は書類取得APIでCSVを取得できないため、facts取り込み対象から除外する |
| facts_ingested_at | DateTime | DATETIME | | | | この書類のCSVを取得し`facts`へ保存済みの日時。`NULL`＝未取得 |
...
| idx_documents_company_ingested | company_code, facts_ingested_at | INDEX | 「この企業の未取得書類」を絞り込むクエリ（facts取り込みバッチ）を高速化 |
...
- **保存対象を絞り込む（YAGNI）**：EDINET全体では大量保有報告書・ファンドの書類等、
  ...
  無制限に膨らむことはない
- **`facts`との関係**：`doc_id`は両テーブルに存在するが、外部キー関係は設けない
  （`documents`は「書類の存在」を表す索引、`facts`は「値そのもの」を表すテーブルで、
  設計上の関心が異なるため）。将来、突合が必要になった場合はアプリケーション側で
  `doc_id`を使ってJOINする
- **`company_code`を`NOT NULL`にする理由**：変換に失敗した書類・`companies`に
  ...
  `company_code IS NULL`の考慮をfacts取り込み処理に持ち込まずに済む
