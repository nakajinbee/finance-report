...
## 追記（2026-07-22）：ユーザーフィードバック対応（データなし調査・B/S・P/L分離・経常利益・自己資本・指標カテゴリ化）

ユーザーが実際にブラウザで確認した際に「データなしが多すぎる」「指標の並びがB/S・P/Lの
構造に沿っていない」「経常利益がない」との指摘を受け、実データで原因調査・追加実装を行った。

### 発見した不具合（データなし調査）
1. **`equity_ratio`のコンテキストID誤り**：自己資本比率は貸借対照表の時点値（Instant）
   なのに、実装は期間値（Duration）で検索していたため開示値を一度も取得できていなかった。
   `DISCLOSED_RATIO_CONTEXT_ID`を指標ごとの辞書に変更して修正
2. **IFRSの配当性向候補が丸ごと欠落**：`PayoutRatioSummaryOfBusinessResults`はJapan GAAPの
   候補リストにしか追加しておらず、IFRS企業（リクルートHD含む）で常にデータなしになっていた。
   実データで同一タグが会計基準によらず共通であることを確認し、IFRSにも追加

いずれもユーザーの実データ（リクルートHD、2018年3月期〜2026年3月期）で修正前後の値を
比較し、正しい値が返るようになったことを確認済み（棚卸資産回転率は業態上正しく
「データなし」のままであることも確認済み）。

### 実装した拡張（FR-27〜28）
- `metric_mappings.FIVE_METRICS`に`ordinary_profit`（Japan GAAPのみ）・`equity`
  （IFRS・Japan GAAP）を追加し、`schemas.FinancialRecord`・`RatioRecord`計算ロジックに反映
- フロントエンドを大幅にリファクタリング：`FinancialChart`・`MetricSelector`を特定の
  指標リストに依存しないジェネリックな実装に変更し、`BS_METRIC_DEFINITIONS`
  （総資産・負債・自己資本）と`PL_METRIC_DEFINITIONS`（売上高・営業利益・経常利益・純利益）
  それぞれに`FinancialMetricSection`として再利用
- 財務分析指標を4カテゴリ（収益性・効率性・安全性・投資指標）に分類し、カテゴリごとに
  トグル＋グラフ＋表を表示する`RatioCategorySection`を新設。投資指標カテゴリ
  （EPS・PER・配当性向）は単位が異なるため2軸グラフ（`RatioCategoryChart`が
  `axis`プロパティに応じて`yAxisId`を出し分け）で対応

### 動作確認
- 10社の実データで`_build_financial_records`の`ordinary_profit`・`equity`を検証
  （IFRS企業は`ordinary_profit=None`、非連結専業の大本組は非連結フォールバックで
  取得できることを確認）
- ユーザーの実データ（リクルートHD、2024〜2026年3月期）でAPI-COM-002・API-COM-005を
  実行し、修正後の`FinancialRecord`・`RatioRecord`のレスポンス形が新しいフロントエンド
  コンポーネントの期待する型と一致することを確認
- `tsc -b --noEmit`・`oxlint`が通ることを確認。開発中のuvicorn（`--reload`）・
  vite（HMR）で変更が自動反映されることを確認（リファクタリング中に一時的なモジュール
  解決エラーがログに出たが、最終状態では解消していることを確認済み）
- 未使用になった`RatioSection.tsx`を削除し、参照が残っていないことを確認

---

## 追記（2026-07-22）：ブラウザ目視確認によるフィードバック対応（軽微な表示修正）

ユーザーがブラウザで実際に確認した結果、2点の表示不備の指摘があり、その場で修正した
（既存FR-27〜28の実装漏れの範囲内の修正のため、新規FR番号は起こさず本サイクルの
追記として扱う）：

1. **財務分析指標（効率性・安全性・投資指標）のグラフに単位表示がなかった**：
   `RatioMetricDefinition`に`unit`フィールド（%・回・円・倍）を追加し、
   `RatioCategoryChart`の`YAxis`に反映した。投資指標カテゴリ（2軸グラフ）は
   右軸に「倍」「%」の2種類の単位が混在するため、`unitsFor()`ヘルパーで
   軸ごとに使われている単位を重複排除して結合表示する（例："倍・%"）
2. **貸借対照表（B/S）・損益計算書（P/L）セクションにグラフはあるが具体的数値の表がなかった**
   （キャッシュフロー・財務分析指標セクションには表があるのに一貫していなかった）：
   `FinancialMetricTable`（`CashFlowTable`と同じパターンの汎用テーブル）を新設し、
   `FinancialMetricSection`のグラフの下に追加した

`tsc -b --noEmit`・`oxlint`が通ることを確認済み。vite HMRでエラーなく反映されることを確認済み。

---

## 追記（2026-07-22）：指標計算の元になった生の金額の表示

ユーザーから「安全性の指標（流動比率・固定比率等）に、計算の元になった流動資産・流動負債・
固定資産・固定負債も表示してほしい」との要望があり、財務分析指標セクション全体に
「指標計算の元になった生の金額」を表示する仕組みを追加した。

- `metric_mappings.BALANCE_SHEET_ITEMS`に`non_current_liabilities`（固定負債）を追加
  （IFRS：`NonCurrentLabilitiesIFRS`、EDINET側のタグ名自体に綴りミスがあり"i"が1つ
  抜けていることを3社で確認済み。Japan GAAP：`NoncurrentLiabilities`）
- `schemas.RatioRecord`・`GET /companies/{code}/ratios`のレスポンスに、安全性・効率性の
  指標計算にのみ使う内訳項目（`current_assets`・`current_liabilities`・
  `non_current_assets`・`non_current_liabilities`・`inventories`）を追加。
  総資産・自己資本・売上高等、既にB/S・P/L表で見える項目は重複させず追加しない
- `RatioMetricDefinition`に`components`（この指標の計算に使った生の金額の一覧）を追加し、
  `RatioCategoryTable`で各指標の行の直前に生の金額の行（グレー表示）を挿入する。
  同じ金額が複数指標で使われる場合（例：固定比率・自己資本比率の両方が使う自己資本）は
  カテゴリ内で最初に登場した箇所にのみ表示し、重複を避ける（表示順の整理）
- 実装中、`<>`（省略記法のFragment）に`key`を付けられないというReactの制約に気づかず
  コンソールエラーを出す実装をしてしまったが、`Fragment`を明示的にimportして
  `key`を渡す形に修正し解消した

10社の実データ・ユーザーの実データ（リクルートHD）で新フィールドの値を確認済み。
`tsc -b --noEmit`・`oxlint`が通ることを確認済み。

---

## 追記（2026-07-22）：内訳（生の金額）をグラフにも表示、初期選択の絞り込み

前回追加した「指標計算の元になった生の金額」は表にのみ表示していたが、ユーザーから
グラフにも表示したいという要望があり、また初期表示では重要な指標（ROE等）だけを
選択状態にし、内訳（純利益・自己資本等）は非選択にしてほしいとの要望があった。

- `lib/ratioCategories.ts`に`CategoryChartEntry`という統一エントリ型を追加し、
  指標本体（%・回・円・倍軸）と内訳（生の金額、億円軸）の両方をグラフの選択肢として
  扱えるようにした（`buildCategoryChartEntries`）。内訳は指標本体の色（Tableau10系）とは
  別系統の色を割り当てて見分けやすくしている
- `RatioCategorySection`の初期選択状態を、指標本体（`isComponent: false`）のみに限定し、
  内訳（`isComponent: true`）は初期状態で非選択にした
- `RatioCategoryChart`・`RatioToggle`を、`RatioMetricDefinition`固有の型ではなく
  `CategoryChartEntry`／汎用エントリを受け取る形に一般化した

実装中、コンポーネントの型シグネチャを段階的に変更していく過程で一時的な不整合による
実行時エラー（`RatioToggle`が古いprops形状を受け取る等）がHMRログに出たが、
全ファイルの変更が完了した最終状態では発生していないことを確認した
（`tsc -b --noEmit`・`oxlint`は最終状態で問題なし）。

---

## 判定：テストフェーズ（実機での最終確認）へ移行可能

サイクル3で残っていた4項目、追加要望（財務分析指標の拡充）、およびユーザーの実機フィードバック
（データなし調査・B/S/P/L分離・経常利益・指標カテゴリ化・単位表示・B/S/P/L表の追加・
指標計算の元になった生の金額の表示・グラフへの内訳表示・初期選択の絞り込み）に対応し、
実機検証・回帰確認を完了した。
