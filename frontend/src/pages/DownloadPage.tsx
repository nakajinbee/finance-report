...
export function DownloadPage() {
  ...
  const [searchResults, setSearchResults] = useState<
    EdinetCompanySearchResult[]
  >([]);
  const [selectedCompany, setSelectedCompany] =
    useState<EdinetCompanySearchResult | null>(null);
  ...
  async function handleDownloadClick() {
    ...
    const result = await startDownload({
      ...
      period:
        periodType === "all"
          ? { type: "all" }
          : { type: "range", from_year: fromYear, to_year: toYear },
    });
    if (!result.ok) {
    ...
    <div className="mx-auto max-w-2xl space-y-6 p-8">
      ...
      <Panel className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          {periodType === "all" && (
            <p className="text-sm text-gray-500">
              EDINETの制約により、実際に取得できるのは最大{MAX_LOOKBACK_YEARS}
              年分です
            </p>
          )}
        </div>

        <Button onClick={handleDownloadClick} disabled={!canDownload}>
          データを取得する
        </Button>
